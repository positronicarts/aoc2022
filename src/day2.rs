...
impl RPSOptions {
    ...
    fn score_against(&self, other: &Self) -> i64 {
        let win = matches!((self, other), 
            (RPSOptions::Rock, RPSOptions::Paper) | 
            (RPSOptions::Paper, RPSOptions::Scissors) |
            (RPSOptions::Scissors, RPSOptions::Rock));

        let draw = matches!((self, other),
            (RPSOptions::Rock, RPSOptions::Rock) | 
            (RPSOptions::Paper, RPSOptions::Paper) |
            (RPSOptions::Scissors, RPSOptions::Scissors));

        self.individual_score() + if win { 6 } else if draw { 3 }  else { 0 } 
    }
}
...
impl aoc22::DayInner<Day2, i64> for Day2 {
    ...
    fn inner(&self, input: String) -> (i64, i64) {
        ...
        println!("{}", lines.len());
        let score: i64 = lines.iter().map(|line| {
